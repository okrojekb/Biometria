...
def unwrap_iris(image, x_center, y_center, rad_pupil, rad_iris,
                liczba_katow=256, theta_mask=None):
    total_height = rad_iris - rad_pupil
    rows = int(total_height)
    pas_wysokosc = total_height // rows

    r_vals = np.linspace(rad_pupil, rad_iris, rows, endpoint=False) + 0.5

    if theta_mask is None:
        theta_zakresy = []
        max_theta_range = np.pi

        for i, r in enumerate(r_vals):
            proporcja = (rad_iris - r) / (rad_iris - rad_pupil)
            proporcja = rad_pupil / r

            zakres_theta = proporcja * max_theta_range

            half_theta = zakres_theta / 2

            theta_start_1 = -half_theta
            theta_end_1 = half_theta

            theta_start_2 = np.pi - half_theta
            theta_end_2 = np.pi + half_theta

            theta_1 = np.linspace(theta_start_1, theta_end_1, liczba_katow // 2, endpoint=False)
            theta_2 = np.linspace(theta_start_2, theta_end_2, liczba_katow // 2, endpoint=False)

            theta = np.concatenate([theta_1, theta_2])
            theta_zakresy.append(theta)

    else:
        theta_zakresy = [theta_mask] * rows

    unwrapped = np.zeros((rows, len(theta_zakresy[0])))

    for i, r in enumerate(r_vals):
        for j, t in enumerate(theta_zakresy[i]):
            x = int(round(x_center + r * np.cos(t)))
            y = int(round(y_center + r * np.sin(t)))
...
def koduj_iris(iris_rect, liczba_pasm=8, liczba_punktow=128, margines=0.2, f=None):
    ...
    pas_wysokosc = wysokosc // liczba_pasm
    r_start = 0
    if f is None:
        f = (0.07 * pas_wysokosc)
    ...
    for i in range(liczba_pasm):
        ...
        f_gabor = 0.1 * len(cechy_1d)
        sigma = 0.5 * np.pi * f_gabor
        x = np.arange(-len(cechy_1d) // 2, len(cechy_1d) // 2)

        gabor_real = np.exp(-x ** 2 / (2 * sigma ** 2)) * np.cos(2 * np.pi * f_gabor * x)
        gabor_imag = np.exp(-x ** 2 / (2 * sigma ** 2)) * np.sin(2 * np.pi * f_gabor * x)
        gabor = gabor_real + 1j * gabor_imag

        odpowiedz = np.convolve(cechy_1d, gabor, mode='same')

        katy = np.angle(odpowiedz)
        cwierci = ((katy + np.pi) // (np.pi / 2)).astype(int) % 4

        kod_binarny = np.unpackbits(cwierci.astype(np.uint8).reshape(-1, 1), axis=1)[:, -2:].reshape(-1)
...
def full_process(canvas1, fig1, canvas2, fig2, canvas3, fig3, label):
    ...
    x_center_cropped, y_center_cropped = x_center, y_center
    ...
    circle = patches.Circle((x_center_cropped, y_center_cropped), rad_cropped, edgecolor='yellow', facecolor='none',
                            linewidth=2)
    ax.add_patch(circle)
    circle2 = patches.Circle((x_center_cropped, y_center_cropped), rad_iris_cropped, edgecolor='pink', facecolor='none',
                             linewidth=2)
    ax.add_patch(circle2)
    fig1.tight_layout()
    canvas1.draw()

    normalized_cropped = unwrap_iris(iris_grey, x_center_cropped, y_center_cropped, rad_cropped, rad_iris_cropped)
    fig2.clear()
    ...
    ax.set_title("Iris code (8 pasów × 256 bitów)")
    ax.axis('off')
    ax.grid(True)
    fig3.tight_layout()
    canvas3.draw()
